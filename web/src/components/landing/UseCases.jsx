          ...
          {useCases.map((useCase, index) => (
            ...
            >
              <div className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-slate-400 mb-6 group-hover:text-slate-600 transition-colors">
                <span>{useCase.audience || 'Use case'}</span>
              ...
              <Link to={`/use-cases/${useCase.slug || generateSlug(useCase.title)}`} className="mt-8 pt-6 border-t border-slate-100 flex items-center text-sm font-bold text-slate-900 opacity-60 group-hover:opacity-100 transition-opacity no-underline hover:text-slate-900">
