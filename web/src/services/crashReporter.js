import { submitClientCrash } from './reportService.js';
...
class CrashReporter {
  ...
  configure({ functions, source = 'web' } = {}) {
  ...
  async send(payload) {
    try {
      await submitClientCrash(this.functions, payload);
    } catch (err) {
      console.error('Crash reporter failed to submit', err);
    }
  }
}
